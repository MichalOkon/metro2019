#ifndef REPO_METRO_SIMPLE_SVG_HPP
#define REPO_METRO_SIMPLE_SVG_HPP
...
namespace svg
{
    ...
    struct Spoint
    {
        Spoint(double x = 0, double y = 0) : x(x), y(y) { }
        double x;
        double y;
    };
    optional<Spoint> getMinPoint(std::vector<Spoint> const & points)
    {
        if (points.empty())
            return optional<Spoint>();
        ...
        return optional<Spoint>(min);
    }
    optional<Spoint> getMaxPoint(std::vector<Spoint> const & points)
    {
        if (points.empty())
            return optional<Spoint>();
    ...
    public:
        ...
        virtual ~Shape() { }
        virtual std::string toString(Layout const & layout) const = 0;
        virtual void offset(Spoint const & offset) = 0;
    protected:
        Fill fill;
        Stroke stroke;
    };
    ...
    {
    public:
        Circle(Spoint const & center, double diameter, Fill const & fill,
    ...
    {
    public:
        Line(Spoint const & start_point, Spoint const & end_point,
             Stroke const & stroke = Stroke())
    ...
    public:
        ...
        Polygon & operator<<(Spoint const & point)
        {
    ...
    {
    public:
        Text(Spoint const & origin, std::string const & content, Fill const & fill = Fill(),
    ...
    private:
        ...
        optional<Dimensions> getDimensions() const
        {
            ...
            optional<Spoint> min = getMinPoint(polylines[0].points);
            optional<Spoint> max = getMaxPoint(polylines[0].points);
            for (unsigned i = 0; i < polylines.size(); ++i) {
        ...
        std::string axisString(Layout const & layout) const
        {
            ...
            axis << Spoint(margin.width, margin.height + height) << Spoint(margin.width, margin.height)
                 << Spoint(margin.width + width, margin.height);
        ...
        std::string polylineToString(Polyline const & polyline, Layout const & layout) const
        {
            ...
            shifted_polyline.offset(Spoint(margin.width, margin.height));
...
#endif //REPO_METRO_SIMPLE_SVG_HPP
