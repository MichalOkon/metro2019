...
void Graph::addStretch(std::string sFrom, std::string sTo, int howMany) {
    int from = stringToID[sFrom], to = stringToID[sFrom];
    Stretch * str = new Stretch(mStretchesAmount++, howMany, stations[stringToID[sFrom]], stations[stringToID[sTo]]);
    graph[from].push_back(str);
    graph[to].push_back(str);
